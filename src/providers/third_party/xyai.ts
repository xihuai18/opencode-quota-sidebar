...
function xyaiConfigCandidates(runtimeProviderID: string, providerID: string) {
...
async function loginAndPersistSession(
  ...
  const response = await fetchWithTimeout(
    ...
    timeoutMs,
  ).catch(swallow("fetchXyaiQuota:login"));
  ...
  if (updateAuth) {
    try {
      ...
      return {
        session,
        warning:
          "session refreshed but failed to persist; using in-memory session",
      } as const;
    }
  }
...
async function fetchQuotaPayload(
  ...
  const response = await fetchWithTimeout(
    ...
    timeoutMs,
  ).catch(swallow("fetchXyaiQuota:quota"));
...
async function fetchXyaiQuota(ctx: QuotaFetchContext): Promise<QuotaSnapshot> {
  const checkedAt = Date.now();
  const runtimeProviderID =
    typeof ctx.sourceProviderID === "string" && ctx.sourceProviderID
      ? ctx.sourceProviderID
      : ctx.providerID;
  const providerConfig = providerConfigFor(
    ctx.config,
    xyaiConfigCandidates(runtimeProviderID, ctx.providerID),
  );
  const siteOrigin = resolveSiteOrigin(
    providerConfig?.baseURL ?? ctx.providerOptions?.baseURL,
  );
...
export const xyaiAdapter: QuotaProviderAdapter = {
  id: "xyai",
  label: "XYAI",
  shortLabel: "XYAI",
  sortOrder: 7,
  normalizeID: (providerID) =>
    providerID === "xyai" || providerID === "xyai-vibe" ? "xyai" : undefined,
  matchScore: ({ providerID, providerOptions }) => {
    if (providerID === "xyai") return 100;
    if (providerID === "xyai-vibe") return 99;
    return isXyaiBaseURL(providerOptions?.baseURL) ? 95 : 0;
  },
  isEnabled: (config) => {
    const renamed = config.quota.providers?.xyai?.enabled;
    if (typeof renamed === "boolean") return renamed;
