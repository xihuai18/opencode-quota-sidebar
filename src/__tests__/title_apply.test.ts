...
describe('title apply', () => {
  ...
  it('stores all used providers in sidebar panel when title view is compact', async () => {
    ...
    const applicator = createTitleApplicator({
      ...
      listCurrentProviderIDs: async () =>
        new Set<string>(['openai', 'anthropic']),
  ...
  it('filters removed providers out of compact title and panel quotas', async () => {
    const config = makeConfig()
    const state = defaultState()
    state.titleEnabled = true

    const createdAt = Date.now()
    const sessionID = 's-filtered'
    const dateKey = dateKeyFromTimestamp(createdAt)
    ...
    const applicator = createTitleApplicator({
      ...
      markDirty: () => {},
      scheduleSave: () => {},
      renderSidebarTitle: (_baseTitle, _usage, quotas) =>
        `Session | ${quotas.map((quota) => quota.providerID).join(',')}`,
      getTitleView: () => 'compact',
      listCurrentProviderIDs: async () => new Set<string>(['anthropic']),
      getQuotaSnapshots: async (providerIDs) => {
        calls.push([...providerIDs])
    ...
    assert.deepEqual(state.sessions[sessionID].sidebarPanel?.quotas, [])
  })
