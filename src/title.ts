...
function isQuotaDecoratedDetail(line: string) {
  ...
  if (
    /^(OAI|Cop|Ant|Kimi|RC(?:-[^\s]+)?)(?:\s+(?:\?|unsupported|unavailable|error|stale|St|(?:\d+h|D|W|M)\d{1,3}|(?:Sk5h|SkW)\d{1,3}|D[\d.,]+\/[\d.,]+|B(?:[￥$-])?[\d.,]+))+$/i.test(
      line,
    )
  ) {
    return true
  ...
  if (
    new RegExp(
      `^(?:Spark\\s+(?:5h|Weekly)\\s+\\d{1,3}%(?:\\s+Rst\\s+${resetValue})?)(?:\\s+stale)?$`,
    ).test(line)
  ) {
    return true
  }
  if (
    new RegExp(
      `^(?:D\\$?[\\d.,]+\\/\\$?[\\d.,]+|B(?:[￥$-])?[\\d.,]+|(?:\\d+[hdw]|[DWM])\\d{1,3}|(?:S7d|O7d|OA7d|Co7d|Sk5h|SkW)\\d{1,3}|St)(?:\\s+(?:R|E\\+?)${compactResetValue})?$`,
    ).test(line)
  ) {
    return true
  }
  if (
    new RegExp(
      `^(OpenAI|Copilot|Anthropic|Kimi|RC(?:-[^\\s]+)?)(?:\\s+(?:(?:Daily\\s+\\$[\\d.,]+\\/\\$[\\d.,]+|\\$[\\d.,]+\\/\\$[\\d.,]+)(?:\\s+(?:Rst|Exp\\+?)\\s+${resetValue})?|(?:\\d+[hdw]|Weekly|Monthly)\\s+\\d{1,3}%(?:\\s+Rst\\s+${resetValue})?)(?:\\s+stale)?|(?:error|unsupported|unavailable|stale))$`,
    ).test(line)
  ) {
    return true
  }
  if (
    new RegExp(
      `^(OpenAI|Copilot|Anthropic|Kimi|RC(?:-[^\\s]+)?)(?:\\s+(?:D\\$?[\\d.,]+\\/\\$?[\\d.,]+|B(?:[￥$-])?[\\d.,]+|(?:\\d+[hdw]|[DWM])\\d{1,3}|(?:S7d|O7d|OA7d|Co7d|Sk5h|SkW)\\d{1,3}|St)(?:\\s+(?:R|E\\+?)${compactResetValue})?)$`,
    ).test(line)
  ) {
    return true
  }
  if (
    new RegExp(
      `^(?:(?:D\\$?[\\d.,]+\\/\\$?[\\d.,]+|B(?:[￥$-])?[\\d.,]+|(?:\\d+[hdw]|[DWM])\\d{1,3}|(?:S7d|O7d|OA7d|Co7d|Sk5h|SkW)\\d{1,3}|St|(?:R|E\\+?)${compactResetValue}))(?:\\s+(?:(?:D\\$?[\\d.,]+\\/\\$?[\\d.,]+|B(?:[￥$-])?[\\d.,]+|(?:\\d+[hdw]|[DWM])\\d{1,3}|(?:S7d|O7d|OA7d|Co7d|Sk5h|SkW)\\d{1,3}|St|(?:R|E\\+?)${compactResetValue})))*$`,
    ).test(line)
  ) {
    return true
  }
  if (
    new RegExp(
      `^(OpenAI|Copilot|Anthropic|Kimi|RC(?:-[^\\s]+)?)(?:\\s+(?:(?:D\\$?[\\d.,]+\\/\\$?[\\d.,]+|B(?:[￥$-])?[\\d.,]+|(?:\\d+[hdw]|[DWM])\\d{1,3}|(?:S7d|O7d|OA7d|Co7d|Sk5h|SkW)\\d{1,3}|St|(?:R|E\\+?)${compactResetValue})))*$`,
    ).test(line)
  ) {
    return true
  }
  if (
    new RegExp(
      `^${legacyProvider}(?:\\s+(?:(?:Daily\\s+\\$[\\d.,]+\\/\\$[\\d.,]+|\\$[\\d.,]+\\/\\$[\\d.,]+)(?:\\s+(?:Rst|Exp\\+?)\\s+${resetValue})?|(?:\\d+[hdw]|Weekly|Monthly)\\s+\\d{1,3}%(?:\\s+Rst\\s+${resetValue})?|(?:error|unsupported|unavailable|stale)))$`,
    ).test(line)
  ) {
    return true
  }
  if (
    new RegExp(
      `^${legacyProvider}(?:\\s+(?:D\\$?[\\d.,]+\\/\\$?[\\d.,]+|B(?:[￥$-])?[\\d.,]+|(?:\\d+[hdw]|[DWM])\\d{1,3}|(?:S7d|O7d|OA7d|Co7d)\\d{1,3}|St|(?:R|E\\+?)${compactResetValue}))*$`,
    ).test(line)
  ) {
    return true
  }
