...
2. 显示支持的订阅制 / 余额制 provider 额度余量（OpenAI Codex、GitHub Copilot、Anthropic、Kimi For Coding、Zhipu Coding Plan、MiniMax Coding Plan、RightCode）
...
- API Cost：按 model `input/output/cache` 单价估算（用于订阅制 provider 的 API 等价成本观察；表示“如果同样的 tokens 按官方 API 售价计费，大约值多少钱”，不是订阅月费）
- API Cost 中 Output 单价乘 `(tokens.output + tokens.reasoning)`（Reasoning 按 output/completion 单价计费）
- quota provider 与 pricing provider 可以分离：runtime / bundled pricing lookup 统一走 `src/cost.ts:canonicalPricingProviderID()`，展示与归因走 `canonicalApiCostProviderID()`；例如 `kimi-for-coding -> moonshotai`、`zhipuai-coding-plan -> zhipu`、`minimax-cn-coding-plan -> minimax`
...
- 定价来源优先使用 OpenCode runtime provider metadata（其上游来自 `models.dev`）；缺失时才回退到插件内置参考价格表。该参考表优先采用官方文档，但在某些 provider（例如 MiniMax）上也可以镜像 OpenCode 共同依赖的 `models.dev` 基线，以保持口径一致
...
- TUI sidebar quota 默认走 compact token：`OAI 5h80 R3h20m`、`Cop M70 R12D00h`、`RC D$31.3/$90 E10h18m`
- 多窗口/多字段在放不下时续到下一行，但**不丢字段**；续行继续使用 compact token，例如：
  - `OAI 5h80 R3h20m W70`
  - `    R2D04h`
- RightCode 的余额或日额度同样走 compact token：`RC D$105/$60 E02-27 B$222.5`
...
| `src/providers/`            | Provider adapters（OpenAI/Copilot/Anthropic/Kimi/Zhipu/MiniMax/RightCode） |
